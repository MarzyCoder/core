    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        ...
        if filename.endswith((".mp4", ".vref")) or host.api.is_hub:
            vod_type = (
                ...
                else VodRequestType.DOWNLOAD
            )
        else:
            vod_type = (
                VodRequestType.NVR_DOWNLOAD if host.api.is_nvr else VodRequestType.RTMP
            )
        ...
        stream = create_stream(
            self.hass, url, {}, DynamicStreamSettings(preload_stream=True)
        )
        stream.add_provider("hls", timeout=3600)  # lange VODs
